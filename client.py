...
class MLLPClient(object):
    ...
    * :py:meth:`MLLPClient.send_message` will wrap the message in the MLLP
    ...
    def send_message(self, message, ack_flag=True):
        ...
        If ack_flag is true, then client waits for acknowledgment from receiver.
        If ack_flag is false, then client won't wait for acknowledgments.
    ...
    def send(self, data, ack=True):
        ...
        If ack is true, then client returns acknowledgment from receiver.
        If ack is false, then client won't wait for acknowledgments.
        """
        # upload the data
